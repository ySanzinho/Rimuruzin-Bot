...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('kiss')
		.setDescription('[SOCIAL] Enviei um beijo para seu amado.')
        .addUserOption(option => option.setName('usuário')
        .setDescription('Usuário que deseja dar beijo.')
        .setRequired(true)
  ),
    async execute(interaction) {
      try {
        ...
        //Busca o gif na API
        const response = await fetch(`https://tenor.googleapis.com/v2/search?q=anime-kiss&key=${tenorApiKey}&random=true`);
        ...
        const kissEmbed = new EmbedBuilder()
          ...
          .setColor('#ff878d')
          .setFooter({
            text: `Beijo enviado por ${author.tag}>`,
